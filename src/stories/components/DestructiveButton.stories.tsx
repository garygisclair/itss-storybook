...
const meta = {
  title: 'Components/Buttons/Destructive Button',
        ...
        <div key={s} style={{ display: 'grid', gridTemplateColumns: '100px repeat(4, 1fr)', gap: 16, alignItems: 'center' }}>
          ...
          {variants.map((v) => (
            <div key={v} style={{ display: 'flex', justifyContent: 'center' }}>
              <DestructiveButton variant={v} size={s} label="Delete" />
            </div>
          ))}
        </div>
      ))}
...
export const Loading: Story = {
  render: () => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16, alignItems: 'flex-start' }}>
      {variants.map((v) => (
        <DestructiveButton key={v} variant={v} size="large" label="Delete" loading />
      ))}
    </div>
  ),
  parameters: {
...
export const Disabled: Story = {
  render: () => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16, alignItems: 'flex-start' }}>
      {variants.map((v) => (
        <DestructiveButton key={v} variant={v} size="large" label="Delete" disabled />
      ))}
    </div>
  ),
  parameters: {
